...
`engine/ops.js` (and `system/vtm5e/ops.js`) the browser uses, validates them by role (the GM
...
```bash
cd worker && npm install && npx wrangler dev --port 8789   # local; the app on localhost talks to it
