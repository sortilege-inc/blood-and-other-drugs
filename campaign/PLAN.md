...
| **U2** | upstream | **D2 — the third-party shelf.** `build_data.py` takes a second corpus root; Black Hand and Sunburners shelved apart and labelled; the two `.arc`s offered as modules | **landed 2026-09-24** — `sortilege-vtt-vtm5e` `f154fdb` + `06d2785`, pulled here. `SHELVES` names each corpus root and, for the third-party repository, the product directories loaded **and** excluded with a reason; `product_dirs()` reads what is on disk and the build raises if the two lists differ, so *Summoned Stories* is excluded **by name**, not by omission. The two `.arc`s are **deferred by name** (`DEFERRED_EXTS`) — their FLOW / PHASE / SCENE / CAST is a construct the reader does not carry, and shelving one as a book would drop the structure that makes it a scenario; offering them in the Chronicle panel is still open. Gate over both roots: `build.sh` green — **155 corpus files, 21 books, 10,565 entities**, verify_data **27,163 strings, 0 uncovered · 0 unsourced**, check_shape **OK, 44 assertions** (38 before; the new ones check the shelves, that each shelf's books read from its own root, and that every deferred file is deferred by a declared extension). The book map still refuses an unclaimed file **and** now a file claimed across shelves. In the browser (0 console errors on every page): the shelf shows three groups — *This campaign*, *The books* (18, hero unchanged at **10,120 entries · 211 powers · 101 rituals · 513 characters**), and *Third-party and homebrew* with its note; a third-party book's crumb reads *The books › Third-party and homebrew › The Black Hand: Playing the Sabbat*; the Disciplines tab now lists **343 from 12 books** and none of the Black Hand's 31 powers and rituals lands in the *not placed* bucket (still 29, all pre-existing); the Cast is **569** with *The Black Hand (7)*. The creator's Predator step is unchanged — `creator.js predators()` is scoped to core + Players Guide (open question below) |
...
| **M2c** | here | **The rest of the record.** The Advantages, Flaws, Equipment and Resonance the world carries on its characters, which M2b's actor pass did not write | **landed 2026-09-24** — the world records **114 feature items on 11 of the 49 characters** (34 merits, 43 backgrounds, 37 flaws), plus one gear and one resonance; M2b wrote none of them, which is a third of a sheet. Now written as `^"Advantages"` (merit + background) and `^"Flaws"`, each entry the world's name then its dots, plus `^"Equipment"` and `^"Resonance"` — all four labels the corpus's own. `check_cast.py` re-derives each from the export's items and compares **(name, dots) pairs**, not the string, so a name carrying its own commas (*Contact (Mike Harrison, Money Launderer)*, *Linguistics (Spanish, English, Russian)*) is compared whole: **3,003 → 3,199 field checks, all match**, and the four new checks **proven by planting six faults** (a dropped Advantage, a changed Flaw rating, a changed Equipment, a changed Resonance, a fabricated Predator Type, a changed Predator Type Note) — six failures, each naming the right field and the right value. Layer gates green: *379 strings (1,096 occurrences) — 0 uncovered · 0 short · 0 unsourced*. **Resolved against the books** after the owner's ruling (decision log): each Advantage written under the book's name with the world's annotation in brackets and referenced by hash — 76 references, all resolve; 23 names in no converted book, listed for the owner. On Ethan Cole's page: **ADVANTAGES** *… Contacts (Mike Harrison, Money Launderer) 2, Dark Secret (Debt) 1, Enemy 1* · **FLAWS** *Bestial Temper 1, Clan Curse (Brujah), Prey Exclusion (Children) 1* · **EQUIPMENT** *Strength of Sadness* · **RESONANCE** *Sanguine*; 0 console errors |
...
* **The two `.arc` scenarios** — *Lost in the Garden* and *Our Graves Are Empty*, the only arcs
  in either corpus. Deferred by name in `DEFERRED_EXTS`, not dropped. Offering them in the
  Chronicle panel needs the reader to carry FLOW / PHASE / SCENE / CAST.
* **The creator's Predator step** offers the 16 official types only (`creator.js predators()`
  reads core + Players Guide). The Black Hand's eight "(Sabbat Only)" types are now in the
  data and resolve on a record, but a player making a character is not offered them. For a
  chronicle with a Sabbat barony that is probably wrong — but it is upstream behaviour for
  every instance, so it is the owner's call, not a silent change.
...
| 2026-09-24 | **owner ruling, corrects a finding of mine** | **Advantages and Flaws are published too, and resolve against the books like every other name.** The owner: *False Love (Trieste)* is the book's *False Love* with the NPC it concerns; *Retainer (Dimitri)* likewise; *Dark Secret: Masquerade Breacher* is *Dark Secret* and the nature of the secret; *The Pony Express* is a loresheet and *Station Agent* its Advantage; *Furcus* a loresheet Advantage; *Knowledge Hungry* and *Weak-Willed Regarding Art* the same. **My earlier finding that "the books do not name them" was wrong**: the books head each Advantage with its dots (*"• False Love"*, *"Knowledge Hungry (•)"*, *"Furcus • to •••"*, *"Business Establishment •• or •••"*) or print it as a field of a list (the core's Thin-blood Flaws: `^"Bestial Temper" STRING …`), and my matcher stripped neither. The Players Guide's *Advantages Master List* holds most of them. **Now:** `corpus_advantages()` reads every heading under a Merits / Flaws / Backgrounds / Advantages heading, every loresheet and level, and every field of a heading that lists them, with the dot notation off, first in shelf order and a chapter before an appendix; the world's annotation is split off — bracketed, or after a colon (the Advantage before it, or the category before it when the part after is itself an Advantage, as in *Haven: Luxury*) — and written in brackets after **the book's** name; each resolved entry is referenced by hash (`"Advantage"` / `"Flaw"`; a field-printed one references the heading that lists it). *Contact* → *Contacts* and *Retainer* → *Retainers* are `ALIASES` (the core heads the Backgrounds plural, as with *Malkavians*); *Weak-Willed Regarding Art* is the one run-on form, an owner-ruled `ANNOTATED` entry — no general "longest prefix" rule, which in the pilot misread *Status Symbol* as *Status* + "Symbol". The homebrew DEF for *False Love* is **gone**: nothing in the world is the chronicle's own rule. **76 references to book entries, all resolve.** `check_cast.py` matches each world item to exactly one written entry by dots and by every form the world might have written it in, refuses anything written that no item accounts for, and checks each name written is one a book prints — its own reading of `data/`, headings and fields, dots on and off: **3,313 checks, all match**; proven by planting four faults (a dropped annotation, a misspelt name, changed dots, an invented Advantage), each named |
| 2026-09-25 | **owner ruling** | **The world's Advantage names that no book prints are mapped to the nearest published one.** Traced first: all 32 items are on the characters in the 2026-09-24 export — 28 typed into the world by hand (no description, no book, no page, no compendium origin), 2 from the `vampire-the-masquerade-5e-compendium` module (*Enemy*, *Shunned by Sire*), 1 copied from a world item (*The Pony Express: Station Agent*). Checked against the Paradox wiki's *Advantages and Flaws* and *Loresheets* pages, which cover books not yet converted: **The Pony Express is real** — a loresheet in *Chicago Folios* (p. 166) and *Live from the Succubus Club* (p. 162), neither converted — and is kept as the world has it until one is. **Enemy** is the core's own Flaw (printed inside its Allies entry; the Players Guide heads it *Enemies*); **Language** is *Linguistics*. The rest appear on neither page. The owner's mapping, as `ALIASES` (renamed) or `ANNOTATED` (the world's name kept as what it is about): Enemy → Enemies · Language → Linguistics · Rival → Adversary · Infamous Deed → Infamy · Iron Will → Tempered Will · Disallowed Feeding → Prey Exclusion · Disallowed Influence → Despised · Efficient Digestion → Iron Gullet · Obsession Target → Stalkers · Status Symbol → Fame · Shunned by Sire → Shunned (by Sire) · Cult → Herd (Cult) · Prestigious Sire → Mawla (Sire). The world's dots are kept: **three differ from the book's** — Prey Exclusion (•) at 2 on Evelyn Skye and Moth, Stalkers (•) at 2 on Lucien. Also noted: *Collector* (Moth, 2) and *Necromantic Prodigy* (Evelyn, 3) resolve by name to loresheet levels the books rate at 4. After a wider search (by what each does, across both corpora and the wiki), the owner took five more: Ritual Savant → **Shrine** (Children of the Blood) · Danger Sense → **Object of Power** (In Memoriam; the world's 2 dots) · Natural Leader → **Embraced to Rule** (the High Clan loresheet; the world's 2 dots, the book's level is 4) · Amnesia → **Dark Secret (Amnesia)** 3 · Vengeful → **Risk-taker** (the world's 2 dots, the book's 1). Viorica's three with no published neighbour were **replaced** at the same dots with Advantages that fit her sheet (owner-approved proposal): Impostor Syndrome → **Dark Secret •••** (the fear of being found out), Inoffensive to Animals → **Library •** (the core's Haven Merit; +1 die to research for one Academics, Investigation or Occult specialty — she has one of each), Nightmares → **Knowledge Hungry •** (Blood Sorcery). Dominik's *Convincing Lies* (Merit 3) — which his Subterfuge (Convincing lies) 5 specialty already carries — **replaced** with **City Secrets •••** (Players Guide Merit; the owner's pick over Influence •••, The Pyramid: Regent and ViennaZero: Instrument of Power). **Every Advantage name in the cast now resolves to a book but one:** *The Pony Express: Station Agent* stays until *Chicago Folios* or *Live from the Succubus Club* is converted |
| 2026-09-24 | autonomous, gate | **`check_cast.py` grew the checks it was missing, and the first thing it caught was mine.** It verified Attributes, Skills, Disciplines, Humanity, Generation, Blood Potency, Health, Willpower, powers, Appearance, Concept and Touchstones — but **not Clan or Predator Type**, the two published names the converter rewrites under O7, nor Chronicle, Sire, Embraced, Ambition, Desire, Convictions or History. Now it checks each rewritten name three ways: that it still points at the world's (aliases, a trailing qualifier and typography aside), that the world's parenthetical went to the Note field rather than into the name, and that the name written is one a book actually prints — read out of `data/` by the checker itself. **2,489 → 3,003 field checks.** It immediately refused ten actors: the converter mapped Foundry's *Thin-Blooded* / *Thin-Blood* to the core's *The Thin-Blooded* as a **special case buried in the code**, where `cast_aliases.py` says in its own header that a name decision "must be reported, not buried". Moved to `ALIASES`, where every run now names it |
| 2026-09-24 | autonomous, method | **A published heading's trailing qualifier is a general rule, not four aliases.** The Black Hand prints its Predator Types *"Absolver (Sabbat Only)"*; Foundry records *"Absolver"*. `corpus_names()` registers the bare form as a **second key, in a second pass, only where that key is still free**, so a qualified heading can never shadow a book that prints the name plainly; the corpus's full spelling is still what gets written and `resolve()` reports the difference on every run. `AWAITING_THIRD_PARTY` is gone — the four names it held resolve with no alias at all |
| 2026-09-24 | finding, **resolved by U2** | **Absolver, Ripper, Domina and Hedonist are published too** — in *The Black Hand*, as "… (Sabbat Only)" Predator Types. Absent from all 19 official books, present in the third-party corpus | Father Vivek is the Sabbat Baron and four Sunburners carry them. They are **not** homebrew and are not written as though they were: the converter kept the world's spelling and named the book on every run until U2 loaded the shelf. **Resolved 2026-09-24:** all six assignments (Father Vivek and Malik the Low *Absolver*, Jonah *Ripper*, Lil Sis *Domina*, Zuzu *Hedonist*) now write the book's own heading — `Hedonist (Sabbat Only)` on Zuzu's page, with the world's *party/rave scene* split off into **Predator Type Note**. **No name in the world is unresolved any more:** the converter's 24 notes are all "read as" or "written as the corpus spells it"; not one "is in no book" |
