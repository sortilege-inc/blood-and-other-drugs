...
| 2026-09-24 | **autonomous, content — flag for the owner** | **A Foundry item's description is usually the publisher's text, so only the owner's own homebrew list becomes a DEF.** 19 of the 46 unresolved feature names carry a description in the world, and reading them shows most are the books' own text pasted in — *"These Merits and Flaws only apply to thin-blood characters…"* is the core's Thin-Blood Merits and Flaws; *"The haven is rented out to a business…"* is its Haven Merits. Writing those into the layer would **republish the publisher's rules as this table's homebrew**, and the world carries no flag telling the two apart. So `cast_aliases.HOMEBREW` is an explicit list, seeded from **O4's own enumeration** and nothing else; of its four names, three are published and resolve, leaving *False Love (Trieste)*. **Open for the owner:** *Knowledge Hungry*, *Weak-Willed Regarding Art*, *The Pony Express: Station Agent*, *Retainer (Dimitri)*, *Dark Secret: Masquerade Breacher* and *Furcus* are the plausible other candidates — each either the table's own or a book's, and I cannot tell which without being told |
| 2026-09-24 | **autonomous, content — flag for the owner** | **Advantages and Flaws are carried verbatim and are NOT resolved against the corpus**, the one published-looking thing in the cast that O7 does not govern. **Why:** the books do not name them. The core's *Advantages* chapter (pp. 179–199, converted and complete) prints its Merits and Flaws as bulleted `Items` under **category** headings — Linguistics, Looks, Substance Use, Archaic, Bonding, Feeding, Mythic, Thin-Blood, Haven Merits and Flaws — while Foundry's wod5e system keeps a flat list of its own. Only **27 of 74** distinct names match a corpus entity, and *Iron Will*, *Efficient Digestion*, *Inoffensive to Animals*, *Shunned by Sire* and *Prestigious Sire* appear **zero times** in any of the 19 official books, case-insensitively, across every file — checked before saying so, and **not a corpus defect**: the corpus is faithful to a book that groups them differently. Rewriting the 27 that do match would also put the book's level notation inside a name (*Eat Food* → the core's *"•• Eat food"*) while the character's own rating sits beside it. **Recommendation, taken:** carry them as the world spells them, say so in the converter's header and here, and resolve nothing. **Trade-off:** a reader cannot click an Advantage through to a book — but there is no entity to click through to, for 47 of the 74 |
