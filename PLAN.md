...
| 29 | **The creator offers the clanless — Caitiff and Thin-blood — from the core's own chapters** (2026-09-25): neither chapter prints a clan heading with a Bane under it, so `clans()` never found them and a thin-blood could not be declared. `VtmData.clanless()` reads *The Caitiff* (its Bane and Disciplines headings) and *The Thin-Blooded* (Clan and Disciplines under Thin-Blood Characteristics; no bane); `thinBloodTraits()` the core's two lists. For a thin-blood each step checks the core's own sentence: no intrinsic Disciplines, no Predator type (the step-by-step chapter), one to three Thin-Blood Merits and as many Flaws (picked from chips; 0-dot lines), no Bonding/Mawla/Retainers/Status, 14th–16th Generation at Blood Potency 0. The Black Hand's walk: decision 30. | Owner: "unclear how to declare a thin blood". |
| 30 | **A Sabbat character is The Black Hand's ACTOR "Sabbat Kindred", made with its own Quick Character Creation where the Storyteller allows it** (2026-09-25). Owner's three calls: the Storyteller turns it on, as a campaign setting like loresheets (`setCreation {blackHand}`, shared, the Storyteller's; a checkbox on the Loresheets panel); the Path lives in the Black Hand corpus, which now declares `ACTOR "Sabbat Kindred" EXTENDS Kindred` with one field, `Path of Enlightenment` (third-party `4a07532`, `4fc0c1a`); and since only a player's page has a campaign, the creator is offered there too (`Sys.makeCharacter`, beside loading a file; the site's creator never offers the Sabbat walk but opens a Sabbat draft in it). The walk is the headings under *Quick Character Creation* (p. 107) read like the core's summary, each mapped to the core step whose checks it shares; the Predator pick is the book's eight Sabbat types, the Path pick every heading carrying a *Path Compulsion* (the five, and Path of the Sun). A character is Sabbat when its values hold the Path field; its sheet then loads The Black Hand (`Sheet.booksFor`) and its file carries the Sabbat Kindred id. check_shape asserts the ACTOR and the nine steps (proved failing on the corpus before). The Worker was redeployed for `setCreation` (version `4ab2f6ed`, 2026-09-25; proved live: the Storyteller sets it, a player receives it and is refused setting it). | Owner: "unclear how to declare … a character that uses the Black Hand sabbat features". |
| 31 | **Sets 5-8 of the bundle are on the shelf** (2026-09-25; corpus `0607701`): 23 books added to BOOKS in the corpus's reading order — the Chicago Folios, Forbidden Religions, Trails of Ash and Bone, the Crimson Gutter, Swansong, A Taste of the Moon, Auld Sanguine, Love Bites, Midnight Kiss, Reins of Power, Under a Changing Moon, the Primogen's Gambit, Under the Skin, Wine-Dark Waters, the Masquerade Companion, the Book of Nod and its Apocrypha, Month of Darkness: Luciana, New Blood's three parts, the Fall of London, and Found Notes. Last Dance at Renauld's is an `.arc` only, so it is DEFERRED like The Black Hand's two. A `.lore` file's page is also read from Found Notes' `*…, page N.*`. 44 books, 14,225 entities, 36,814 strings 0 uncovered, check_shape 46. | Owner: "the new sourcebooks are available now to bring in". |
| 32 | **Third-party options are the player's to take, on the main creator; The Black Hand adds to the core walk** (2026-09-25, superseding decision 30's separate walk and its player's-page-only offer). The creator carries a *Use third-party options* checkbox, the third-party shelf's books to use (The Black Hand; the Sunburners' Path, which needs it) and an acknowledgment that the character is playable only where the Storyteller allows those sources; with all three the draft is a Sabbat Kindred. The core's nine steps stay: The Black Hand's eight Sabbat Predator types join the Predator pick, and a Path of Enlightenment (optional) joins Convictions and Touchstones, with its Quick Character Creation text for those two steps shown beside the core's. The Storyteller's `setCreation {blackHand}` now says whether the table *allows* them: the party list marks a Black Hand character, and "(not allowed at this table)" until it does; a player loading one is told. No op changed. | Owner: "I should be able to do it from the main making a character menu … The Black Hand character creation adds options to the core character creation, it doesn't fully replace it". |
| 33 | **Loresheets are a filter in the Advantages finder, beside type and source** (2026-09-25): step 0 keeps only *Use loresheets* and its acknowledgment; the finder has type chips (All · Merits · Backgrounds · Flaws · Loresheets, the last only when step 0 took them up) and a Source menu (every book that prints one); a loresheet result lists its levels, each taken on its own. A name printed in several books is one entry citing each, its dots from whichever book prints them (the core's *Allies* heading has none; the Players Guide's summary line has them). No Advantage offers more than five dots | Owner: "loresheets should show as a filter in the same interface as the rest, not in a huge list beforehand … filter the list by type and by source"; on Allies: "5 is the max" — the Players Guide prints "• to ••••••", kept verbatim in the corpus and logged there as an owner override to check. |
| 34 | **The Sea of Time spends experience at the core's prices** (2026-09-25): the step's bands (Childer · Neonates · Ancillae) are cards read from the summary; a band offers its Generation / Blood Potency lines (a thin-blood's only to a thin-blood), then, where it gives experience, a budget with a buy button per trait priced by `VtmAdvance.costs()` (the core's Trait Costs table) and `disciplineRate` (in-clan / other / Caitiff) — Attributes, Skills, Specialties, Disciplines, Advantages, Blood Potency. Purchases sit in `meta.xpBuys`, each undone on its own; changing the band takes them all back (after a confirm). Checks: a band chosen, the band's experience spent exactly, its Advantages / Flaws / Humanity counted in the other steps' checks; dots bought with experience are not counted against the book's 7 Advantage points | Owner: "Sea of time should include a process to spend XP to advance things according to the rules, and display costs in the interface". |
| 35 | **data/ rebuilt on titterpig-dsl-vtm5e `56a978e`** (every book's Advantages typed; spaced headings repaired). Advantage records come from every Merit/Flaw/Background/Advantage `EXTENDS`, never from BASE's own type DEFs; `renamed_ids.py` now keeps the renames already shipped (chaining any whose target moved again) and matches a re-spaced heading by its letters when exactly one id went and one came | `build.sh` → 336 files, 44 books, 14257 entities, 475 advantage records, `check_shape: OK (48 assertions)`; `renamed-ids.js` 262 ids (58 carried + 204), 28 fragments with no successor; 14 headless harnesses + `creator-focus-check.js` → 0 errors. |
| 36 | **The creator's polish round** (owner, 2026-09-25): power chips on the sheet with room around their × ("the X is hugging the text too closely"); the Predator step's types as cards with the book's own description, the chosen one whole ("I'm not seeing a description of the predator types anywhere"); step 0 lists every book that offers character options with a toggle and what it offers — the core and the Players Guide always, the walk reads them — and the Advantages finder, the loresheets and the Discipline powers follow it (`bookOn`), third-party books only when taken up ("Sources should allow you to toggle any of the sources made available"); the Disciplines tab filters by name or any printed word, Discipline, level, kind and book, redrawing only its list so the search keeps focus ("the disciplines section should be filterable") | ui2.js, chips.js and the 15 harnesses + creator-focus-check: 0 errors. |
| 37 | **Powers are read by their declared type, not their shape** (titterpig-dsl-vtm5e `34ee7b0`, BASE 0.5.6): `TYPED_KINDS` gains Discipline Power → power, Ritual / Ceremony / Formula → ritual, each with its declared Discipline and Level (a printed "LEVEL 1 OR 3" at its first, with `printedLevel`); SHAPES keeps only characters. The Disciplines tab's "Not placed" list is empty and shows only when the corpus leaves one | Owner: "wtf is going on with the 37 that can't be mapped … fix that shit in the corpus". `build.sh` → 267 powers + 213 rituals (372 before), `check_shape: OK (47 assertions)`, each against its EXTENDS lines; wiki 383 of 391 as placed; `renamed-ids.js` 274 ids, 161 gone with no successor (all field sub-headings folded into properties). |
| 38 | **Every Predator type the books print, from the books a character draws on** (titterpig-dsl-vtm5e `742441b`, BASE 0.5.7): a `predator` record per typed Predator type (29, check_shape against its EXTENDS lines); `predators(v, meta)` reads them for the books step 0 has on (The Black Hand's only for a Sabbat character), a name printed twice offered once from the first book in shelf order, a book loaded when its types are needed. The grant reader takes "Add a specialty" without its colon, "Add one dot of", "N additional dots of", "the Flaw X" / "an X", a Flaw's rating from its record when the grant prints none, and "N dots of A or B, …" as a choice | Owner: "continue with addressing the extra predator types". 18 offered (the wiki's 18; 16 before): Roadside Killer and Tithe Collector new; the other 16 types' grant buttons identical to before (compared against HEAD); pred.js, pred2.js and the 16 harnesses + focus check: 0 errors. |
| 39 | **An entity's other names (spec ALIAS) are kept and found** (2026-09-26): `build_data` keeps an entity's `ALIAS` strings as `aliases` (and on its record); the entity header shows "Also called …", the site search and the Advantages finder match either name. First use: Cults' loresheet The Nation of Blood, printed Descendants of the Baron (titterpig-dsl-vtm5e owner ruling) | Owner: "both names should work but The Nation of Blood is the new term that should be used primarily". verify_data caught the dropped alias before the parser kept it; build.sh OK (48); alias.js: both names find it; 10 harnesses + focus check 0 errors. |
| 36 | **data/ rebuilt on titterpig-dsl-vtm5e `94f79bf`** (Set 9: *Live from the Succubus Club* and *Winter’s Teeth Vol. 1* on the shelf; "Lyle Mercer Cochrane III" for "Iii" and Cults' Handout #3 "will" — owner rulings, 2026-09-25). Two BOOKS entries added after Found Notes. The Cochrane heading's rename moved its DEF and five subsections to new ids (path-derived); `renamed_ids.py HEAD` maps all six | `build.sh` → 346 corpus files, 46 books, 15112 entities, records power 260 · ritual 112 · character 803; verify 38522 strings 0 uncovered · 0 unsourced; `check_shape: OK (48 assertions)`; `renamed-ids.js` 268 ids (262 carried + 6), 0 with no successor; a campaign stored with Cochrane's old ids (a scene cast, a notes key) loads through engine/state.js as the new ids ("2 renamed id(s) carried over"). Blood & Other Drugs names the six only in its own data copy, so it carries them over when it next merges upstream |
...
## V11 — the family standards (owner, 2026-09-24; PLAYBOOK §4b) — 2026-09-25

From the kit (`~/Sortilege/VTT/tools/port-standards`: `port_standards.py`, `port_config.py`) and,
because this VTT already had system-level GM panes, rebuilt in `system/vtm5e/` on
`sortilege-vtt-l5r5e` I19 by way of `sortilege-vtt-tor2e` `e8b6b23`. Asked for by the owner through
*Blood & Other Drugs*' M3; the VtM5e VTT session confirmed nothing in flight first.

| # | What changed | Proof |
|---|---|---|
| V11 | **Not crawled:** `robots.txt` (the AI crawlers by name, then `*`) and the robots meta on `index.html`, `gm/index.html`, `gm/vtt.html`, `gm/play.html`, `gm/maps.html`. **The Storyteller's material in the GM tabs, in the pack:** `engine/gm-text.js` (the GM Markdown with SET / AGREED / SOURCE / YOURS / OPEN / MINE / NOTE tags, section lists with an editor, a search) and `engine/gm-panes.js` (Overview with rulings and free notes, Places, Settings — it registers only what the system lacks); `system/vtm5e/gm-panes.js` rebuilt — **Scenes** grouped by session, a card per scene with a one-line summary and folding beats, a played session folded to a line, the **questions for the table**, and VtM5e's own **Play it** (an arc scene into the Chronicle, its summary and text as the scene's Storyteller's notes); **Threads · NPCs** as sections with what happened to each in play, closed / reopened, and the current scene's cast with what their records print (kept); **People** with an *About* picker (a Storyteller character from any book or an instance's layer, or a member of the Coterie) whose sections show in the **Inspector** and on the **Coterie** card; **Loresheets** and **Notes** unchanged. `setGm` (new), `setGmNotes`, `setArc`, `setThreads` are **local ops** (`{ local: true }`, never sent to a session's room); `setLoresheets` stays shared (the players' sheets read it). `engine/state.js`'s seed fills by id and never re-adds what the Storyteller removed. **A gate in front of `/gm/`** (`gmGate`, once per tab, in the Storyteller's words). **The books off the public site** (`siteBooks: false`; the six book tabs — The books, Clans, Disciplines, Making a character, Storyteller characters, Search — marked `books: true`, the dice not), turned on per browser in Settings | **landed 2026-09-25** — `build.sh` against the corpora's **committed** HEADs (official `3d0cf93`, third-party `324a2da`, exported read-only; the official working tree holds an uncommitted conversion of ~180 files that no book in the map claims yet, which the gate rightly refuses — not this change's to shelve): *155 files, 21 books, 10,572 entities*, *27,034 strings 0 uncovered · 0 unsourced*, **check_shape OK (44)**, and **`data/` byte-identical** to what was committed. In the browser, on the *Blood & Other Drugs* instance (8743), fresh loads: the public site with books off → *Dice* only (the campaign's tabs come with its M3); `robots.txt` 29 `User-agent` lines ending `*` / `Disallow: /`, the meta on every page; `/gm/` → the **gate** in the instance's words → **Enter** → 19 panes, 0 console errors; **Settings** → *Show the books…* → the site's six book tabs back, then off again. Through the controls: Overview *The premise* saved with `SET 25 Sep` and `OPEN` drawn and **bold** rendered; Scenes *The frat house lab* added (opens in its editor), session *Session 5*, a summary, `SOURCE`/`OPEN` text, a beat *The fire-breather*, saved; **Play it** → a Chronicle scene *The frat house lab*, now current, its notes the summary + text, the button now *Go to it*; People *Rose's past* **About Rose** (found in the instance's own layer) `MINE` → shown in **Rose's Inspector** under *GM notes · never sent to players*; Threads *The alley diablerist* with its in-play note, **Close** → *0 open, 1 closed*. `gm/vtt.html`, `gm/play.html`, `gm/maps.html`: **0 console errors**. **Local ops, live** (the instance's Worker under `wrangler dev`, a room started from the GM page, `WebSocket.send` counted): `setGm`, `setGmNotes`, `setArc`, `setThreads` → **0 frames**; `setLoresheets` → 1. The test state was removed afterwards |

| # | Decision | Why |
|---|---|---|
| V11-1 | The gate says *Storyteller*, not the kit's *GM*; the kit's second title line in `engine/app.js` dropped (the page already set its title) | V5's word; a duplicate. |
| V11-2 | `engine/gm-text.js` loads before `system/vtm5e/gm-panes.js` (the kit put it after) | The system's panes use it (TOR2e F-3). |
| V11-3 | **`gm/maps.html` is not gated** (it has the robots meta) | Players open it from their own page to draw on shown maps (V9), as they do `gm/play.html`; a gate would stop them, not warn them. |
| V11-4 | VtM5e's own People pane, with an *About* picker over its `character` records and the Coterie | The engine's People editor cannot say whom a section is about (TOR2e F-4); the shared kit is not edited. |
| V11-5 | **The Worker is not redeployed** by this change | The ops that changed are local and never reach a room, so the deployed Worker (`6e64cfdd`) sees no op it does not know; redeploying is the owner's call |
