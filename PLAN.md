...
| M4 | The character sheet derived from the ACTOR, the creator (the core's *Character Creation* walked), the live trackers | **waits on D1** — the corpus declares no character. Until then the coterie is held as name, player and Hunger (decision 12). |
...
**M0–M3 and M5 landed 2026-09-23**, each committed and pushed; **M4 waits on D1** (owner).
Nothing is deployed: the repo is private, `engine/config.js worker.deployed` is empty (D3).

To resume: `bash build/build.sh` (gate green), start the launch entry `vtt-vtm5e` (8738) and
open `/` and `/gm/`; for sessions also `vtt-vtm5e-worker` (8789; `worker/` has `node_modules`)
and test a player from `http://127.0.0.1:8738/gm/play.html?s=CODE`.

**When D1 is decided:** add the ACTOR to `titterpig-dsl-vtm5e/0.5/vtm5e-0.5-base.ttrpg` (own
commit, VERSION bump, `./gates.sh` green), rebuild here, then write `system/vtm5e/sheet.js`'s
derived sheet (the Troika and Invisible Sun `sheet.js` are the pattern) and the creator.
**When D2 is decided:** add the third-party corpus root to `build_data.py` (a second BOOKS list
with its own prefix map) and offer its two arcs in the Chronicle panel.
**When D3 is decided:** `cd worker && npx wrangler deploy`, set `worker.deployed` and the Worker's
`ALLOWED_ORIGIN`, make the repo public, enable Pages from `main`, add the CNAME.
